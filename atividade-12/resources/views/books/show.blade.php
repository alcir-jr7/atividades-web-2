                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="user_id" name="user_id" required>
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ $user->hasDebit() ? 'disabled' : '' }}>
                                {{ $user->name }}
                                @if($user->hasDebit())
                                    (Débito: R$ {{ number_format($user->debit, 2, ',', '.') }})
                                @endif
                            </option>
                        @endforeach
                    </select>
                    <small class="text-muted">Usuários com débitos não podem realizar empréstimos.</small>
                </div>
